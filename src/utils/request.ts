...
type RequestOptions<T extends keyof RequestResponse> = {
  url: string;
  data?: any;
...
export function Request<T extends keyof RequestResponse>(options: RequestOptions<T>): Promise<RequestResponse[T]> {
  return new Promise((resolve, reject) => {
    ...
    const req = protocol.get({ path, port, host, method: options.method, headers: options.headers }, (res) => {
    ...
    if (options.data) {
      ...
      req.write(ItemToString(options.data));
    };
