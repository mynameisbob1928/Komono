...
export type PrefixResolvedItem<T extends PrefixItem> =
    T extends PrefixArgRole ? Role
  : T extends PrefixArgUser ? T["isMember"] extends true ? GuildMember : User
  : T extends PrefixArgString ? T["choices"] extends Record<string, string> ? { target: keyof T["choices"], value: string; } : string
  : T extends PrefixArgNumber ? T["choices"] extends Record<string, number> ? { target: keyof T["choices"], value: number; } : number
  : T extends PrefixArgBoolean ? boolean
  : T extends PrefixArgChannel ? T["channelType"] extends Array<keyof typeof ChannelType> ? (typeof ChannelType)[T["channelType"][number]]: (typeof ChannelType)[ChannelType]
  : never;
